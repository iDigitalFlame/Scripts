...
S_SPEC = {"WindowsDC": 0, "Linux": 0, "NetDevice": 0, "Printer?": 0}
S_DESC = {
    "80": "Web Server",
    "443": "Web Server",
...
def _dohorse(tfile):
    ...
    for row in csvd:
        ...
        if not ("%s:%s" % (row["Port"], row["Protocol"])) in host[
            row["Host"]
        ].ports and (row["Port"] != 0):
    ...
    for k, v in host.iteritems():
        ...
        if "445:tcp" in v.ports and "53:udp" in v.ports:
            wo.write("Windows DC!\n")
            S_SPEC["WindowsDC"] += 1
        if "22:tcp" in v.ports and "445:tcp" not in v.ports:
            wo.write("Linux Box!\n")
            S_SPEC["Linux"] += 1
        if "23:tcp" in v.ports and "22:tcp" in v.ports:
            wo.write("Network Device!\n")
            S_SPEC["NetDevice"] += 1
        if "23:tcp" in v.ports and "161:udp" in v.ports and "22:tcp" not in v.ports:
            wo.write("Network Device!\n")
            S_SPEC["NetDevice"] += 1
        if "22:tcp" in v.ports and "161:udp" in v.ports and "23:tcp" not in v.ports:
            wo.write("Network Device!\n")
            S_SPEC["NetDevice"] += 1
        if "21:tcp" in v.ports and "23:tcp" in v.ports and "445:tcp" not in v.ports:
            wo.write("Printer??\n")
            S_SPEC["Printer?"] += 1
        wo.write("=========================================\n")
    wo.write("============---------------------===========\n\n\n")
