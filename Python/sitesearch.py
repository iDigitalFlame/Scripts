...
class SiteSearch(threading.Thread):
    def __init__(
        ...
        if self.__dns:
            ...
            if "http" in dnshn:
                dnshn = (
                    dnshn.replace("https:", "").replace("http:", "").replace("//", "")
                )
                if "/" in dnshn:
                    dnsind = dnshn.find("/")
                    dnshn = dnshn[:dnsind]
            if self.__debug:
                print(
                    "Search[%s] Using dns server '%s' to find ip address for '%s'"
    ...
    def run(self):
        ...
        if self.__loginparams and self.__loginpage:
            if self.__debug:
                print(
                    "Search[%s] Login page present, attempting to login." % self.baseurl
                )
            self.__loginintopage()
        # Loop while we have things to search
        while len(self.__search) > 0 and (
            self.__maxpages == 0 or len(self.pages) < self.__maxpages
        ):
            ...
            if self.__debug:
                print(
                    "Search[%s] Popped out page '%s', '%d' left."
                    % (self.baseurl, searchpage, len(self.__search))
                )
            ...
            if searchpage and not self.__isscanned(searchpage):
                if self.__debug:
                    print(
                        "Search[%s] Page '%s' is ok to search! running.."
                        % (self.baseurl, searchpage)
                    )
    ...
    def password(self):
        if self.__loginparams:
            return self.__loginparams[self.__loginpasswordfm]
        return None
    ...
    def username(self):
        if self.__loginparams:
            return self.__loginparams[self.__loginusernamefm]
        return None
    ...
    def __loginintopage(self):
        ...
        if self.__debug:
            print(
                "Search[%s] Trying login page '%s' with creds provided."
                % (self.baseurl, lurl)
            )
        # Do login (or try to)
        if self.__ipaddr:
        ...
        try:
            loginpage = requests.post(
                lurl, data=self.__loginparams, timeout=self.__timeout, stream=True
            )
        # Timeout
            ...
            if self.__debug:
                print(
                    "Search[%s] [ERROR] Page '%s' failed to download (time out)!."
                    % (self.baseurl, lurl)
                )
        ...
        # Failure
            ...
            if self.__debug:
                print(
                    "Search[%s] [ERROR] Page '%s' failed to download (refused)!."
                    % (self.baseurl, lurl)
                )
            self.__failed.append(lurl)
            return
        ...
        if not loginpage:
            if self.__debug:
                print(
                    "Search[%s] [ERROR] Page '%s' failed to download!."
                    % (self.baseurl, lurl)
                )
            self.__failed.append(lurl)
            return
        ...
        if loginpage.status_code != 200:
            if self.__debug:
                print(
                    "Search[%s] [ERROR] Page '%s' failed by status code."
                    % (self.baseurl, lurl)
                )
            self.__failed.append(lurl)
            return
        ...
        if not ("text" in loginpage.headers["content-type"]):
            if self.__debug:
                print(
                    "Search[%s] [ERROR] Page '%s' is not a text file."
                    % (self.baseurl, lurl)
                )
            self.__failed.append(lurl)
            return
        ...
        if loginpage and loginpage.request.headers:
            if self.__debug:
                ...
                print(
                    "Search[%s] Login went through, cookie '%s' saved."
                    % (self.baseurl, cookie)
                )
            # Did we get a 200?
            if loginpage.status_code != 200:
    ...
    def __pollpage(self, url):
        ...
        if not geturl:
            if self.__debug:
                print(
                    "Search[%s] [ERROR] Page '%s' failed scope test."
                    % (self.baseurl, geturl)
                )
        ...
        try:
            pagedata = requests.get(
                geturl, headers=self.__sessiond, timeout=self.__timeout, stream=True
            )
        # Timeout
        except requests.exceptions.ReadTimeout:
            if self.__ipaddr:
        ...
        if geturl in self.pages:
            ...
            try:
                ...
                if self.__debug:
                    print(
                        "Search[%s] [ERROR] Page '%s' failed with an IO error."
                        % (self.baseurl, geturl)
                    )
                self.__failed.append(geturl)
                return
    ...
    def __checkpageurl(self, url):
        # Out of range?
        if (
            url.startswith("http") and (not url.startswith(self.baseurl))
        ...
        if rurl.startswith("./"):
            rurl = rurl[2:]
        # Fix for sites that use full absolute paths (Looking at you dokuwiki!)
        # Get 'token' (base url path)
        token = self.baseurl.replace("http://", "").replace("https://", "")
        # See if we have a trailing slash
        if "/" in token:
            # find index of token
            itoken = token.find("/")
            # Do we have a token?  Lets remove it
            if rurl.startswith(token[itoken:]):
                return self.baseurl.replace(token[itoken:], "") + rurl
            ...
            for link in alist:
                ...
                if href:
                    ...
                    if not href:
                        if self.__debug:
                            print(
                                "Search[%s] [ERROR] Link '%s' is not in scope!."
                                % (self.baseurl, link.get("href"))
                            )
                    else:
            ...
            if dlist:
                ...
                for divs in dlist:
                    ...
                    for word in wrdlst:
                        ...
                        if divcount < self.__keywords and random.randrange(
                            0, 10 * len(wrdlst)
                        ):
...
class JSONLoader(threading.Thread):
    ...
    def run(self):
        ...
        # self.__ssearch.baseurl = self.__host
        self.__ssearch.start()
        # ssearch.start()
        # Wit for completion
        # while ssearch.isAlive():
        while self.__ssearch.isAlive():
            pass
        # Post results
        print(
            "Site '%s' Finished! %d results returned."
            % (self.__host, len(self.__ssearch.pages))
        )
        self.result = self.__ssearch.json()
        self.resultu = self.__ssearch.username()
        self.resultp = self.__ssearch.password()
...
def dnslookup(name, dns):
    ...
    if name and dns:
        try:
            ...
            dnsr = DNS.DnsRequest(
                name, qtype="A", server=[dns], protocol="udp", timeout=30
            )
            # Process our request
            request = dnsr.req()
...
def loadfromjson(jsonpath, forms=False, debug=False, keywords=6, maxpages=10):
                ...
                if "index" in service:
                    for thread in searchlist:
                        ...
                        if thread.index == service["index"]:
                            ...
                            if thread.resultu:
                                service["username"] = thread.resultu
                            if thread.resultp:
                                service["password"] = thread.resultp
...
if __name__ == "__main__":
    ...
    aaa = SiteSearch("http://www.alpha.net/", dns="10.100.101.100", debug=True, forms=False,
                     maxpages=10, keywords=6, loginpage="/index.php?main_page=login&amp;action=process&amp",
                     username="scorebot@alpha.net", usernamefm="email_address", passwordfm="password",
                     password="Orange4")
    aaa.start()
    while aaa.isAlive():
        None
    ...
    loadfromjson("V:\\bsideslv2015.bak")
